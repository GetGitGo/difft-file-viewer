...
use crate::summary::SyntaxBlock;

/// Collect `List` syntax nodes as gutter-selectable blocks for JSON export.
pub(crate) fn collect_syntax_blocks<'a>(roots: &[&'a Syntax<'a>]) -> Vec<SyntaxBlock> {
    let mut blocks = Vec::new();
    for root in roots {
        walk_syntax_blocks(root, &mut blocks);
    }
    blocks
}

fn walk_syntax_blocks<'a>(node: &'a Syntax<'a>, blocks: &mut Vec<SyntaxBlock>) {
    match node {
        List {
            open_content,
            open_position,
            ...
            if let Some((start_line, end_line)) =
                list_line_span(open_position, close_position)
            {
                let label = list_label(open_content, close_content);
                if !label.is_empty() || start_line != end_line {
                    blocks.push(SyntaxBlock {
                        id: node.id().get(),
                        parent_id: node.parent().map(|p| p.id().get()),
                        label,
                        start_line,
                        end_line,
                    });
            ...
            for child in children {
                walk_syntax_blocks(child, blocks);
            }
        }
        Atom { .. } => {}
    }
...
fn list_label(open_content: &str, close_content: &str) -> String {
    if open_content.is_empty() {
        close_content.to_owned()
    } else if close_content.is_empty() {
        open_content.to_owned()
    } else {
...
fn list_line_span(
    open_position: &[SingleLineSpan],
    close_position: &[SingleLineSpan],
) -> Option<(u32, u32)> {
    let start = open_position
        .first()
        .or_else(|| close_position.first())?;
    let end = close_position
        .last()
        .or_else(|| open_position.last())?;
    Some((start.line.0, end.line.0))
}
