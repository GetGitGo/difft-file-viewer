...
fn app() -> clap::Command {
    ...
    Command::new("Difftastic")
        ...
        .arg(
            Arg::new("dump-syntax-blocks")
                .long("dump-syntax-blocks")
                .value_name("PATH")
                .action(ArgAction::Set)
                .long_help(
                    "Parse a single file and output syntax blocks as JSON (for GUI gutter selection).",
                )
                .help_heading("DEBUG OPTIONS"),
        )
        .arg(
...
pub(crate) fn parse_args() -> Mode {
    ...
    if let Some(path) = matches.get_one::<String>("dump-syntax-blocks") {
        return Mode::DumpSyntaxBlocks {
            path: path.to_owned(),
            ignore_comments,
            language_overrides,
        };
