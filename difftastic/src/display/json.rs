...
use crate::summary::{DiffResult, FileContent, FileFormat, SyntaxBlock};
...
struct File<'f> {
    ...
    status: Status,
    lhs_syntax_blocks: &'f [SyntaxBlock],
    rhs_syntax_blocks: &'f [SyntaxBlock],
}

impl<'f> File<'f> {
    fn with_sections(
        summary: &'f DiffResult,
        aligned_lines: Vec<(Option<u32>, Option<u32>)>,
        chunks: Vec<Vec<Line<'f>>>,
    ) -> Self {
        File {
            ...
            lhs_syntax_blocks: &summary.lhs_syntax_blocks,
            rhs_syntax_blocks: &summary.rhs_syntax_blocks,
        }
    }

    fn with_status(summary: &'f DiffResult, status: Status) -> Self {
        File {
            language: &summary.file_format,
            path: &summary.display_path,
    ...
    fn from(summary: &'f DiffResult) -> Self {
        match (&summary.lhs_src, &summary.rhs_src) {
            (FileContent::Text(lhs_src), FileContent::Text(rhs_src)) => {
                ...
                if hunks.is_empty() {
                    return File::with_status(summary, Status::Unchanged);
                }

                if lhs_src.is_empty() {
                    return File::with_status(summary, Status::Created);
                }
                if rhs_src.is_empty() {
                    return File::with_status(summary, Status::Deleted);
                }
                ...
                File::with_sections(summary, aligned_lines, chunks)
            }
            (FileContent::Binary, FileContent::Binary) => {
                ...
                File::with_status(summary, status)
            }
            (_, FileContent::Binary) | (FileContent::Binary, _) => {
                File::with_status(summary, Status::Changed)
            }
        }
    }
}
...
impl Serialize for File<'_> {
        ...
        S: Serializer,
    {
        ...
        // + lhs_syntax_blocks / rhs_syntax_blocks if not empty (0-2)
        let field_count = 3
            + (!self.aligned_lines.is_empty()) as usize
            + (!self.chunks.is_empty()) as usize
            + (!self.lhs_syntax_blocks.is_empty()) as usize
            + (!self.rhs_syntax_blocks.is_empty()) as usize;
        ...
        if !self.lhs_syntax_blocks.is_empty() {
            file.serialize_field("lhs_syntax_blocks", self.lhs_syntax_blocks)?;
        }
        if !self.rhs_syntax_blocks.is_empty() {
            file.serialize_field("rhs_syntax_blocks", self.rhs_syntax_blocks)?;
        }
...
#[derive(Serialize)]
struct SingleFileSyntaxBlocks<'f> {
    path: &'f str,
    language: String,
    syntax_blocks: &'f [SyntaxBlock],
}

pub(crate) fn print_syntax_blocks(
    path: &str,
    language: &FileFormat,
    syntax_blocks: &[SyntaxBlock],
) {
    let file = SingleFileSyntaxBlocks {
        path,
    ...
    println!(
        "{}",
        serde_json::to_string(&file).expect("failed to serialize syntax blocks")
    );
}
