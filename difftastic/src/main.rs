        ...
        Mode::DumpSyntaxBlocks {
            ...
            match language {
                Some(lang) => {
                    ...
                    let ast = tsp::parse(&arena, &src, &ts_lang, ignore_comments);
                    init_all_info(&ast, &[]);
                    let blocks = syntax::collect_syntax_blocks(&ast);
                    display::json::print_syntax_blocks(
                        &display_path,
                        &FileFormat::SupportedLanguage(lang),
                        &blocks,
                    );
                }
                None => {
...
fn diff_file_content(
    ...
    let (file_format, lhs_positions, rhs_positions, lhs_syntax_blocks, rhs_syntax_blocks) =
        match lang_config {
        None => {
            ...
            match tsp::to_tree_with_limit(diff_options, &lang_config, lhs_src, rhs_src) {
                Ok((lhs_tree, rhs_tree)) => {
                    match tsp::to_syntax_with_limit(
                        ...
                        Ok((lhs, rhs)) => {
                            let lhs_syntax_blocks = syntax::collect_syntax_blocks(&lhs);
                            let rhs_syntax_blocks = syntax::collect_syntax_blocks(&rhs);
